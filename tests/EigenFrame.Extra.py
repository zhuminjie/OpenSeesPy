...
for eleType in eleTypes:
 ...
 #   b 1.5 d 2.0
    ...
    patch( 'quad', 1,numFiberY,numFiberZ,-y/2.0,-z/2.0,y/2.0,-z/2.0,y/2.0,z/2.0,-y/2.0,z/2.0)
    ...
    # add column element
    transfTag = 1
    geomTransf( coordTransf, transfTag)
    integTag1 = 1
    beamIntegration( 'Lobatto', integTag1, 1, nPts)
    integTag2 = 2
    beamIntegration( 'Lobatto', integTag2, 2, nPts)
    eleTag = 1
    for i in range(numBay+1):
        end1 = i+1
        end2 = end1 + numBay +1
        for j in range(numFloor):
            ...
            if eleType == "elasticBeam": 
                ...
                element('forceBeamColumn', eleTag, end1, end2, transfTag, integTag1, '-mass', M)
            elif eleType == "dispBeamElasticSection": 
                element('dispBeamColumn', eleTag, end1, end2, transfTag, integTag1, '-mass', M, massType)
            elif eleType == "forceBeamFiberSectionElasticMaterial":
                element('forceBeamColumn', eleTag, end1, end2, transfTag, integTag2, '-mass', M)
            elif eleType == "dispBeamFiberSectionElasticMaterial":
                element('dispBeamColumn', eleTag, end1, end2, transfTag, integTag2, '-mass', M, massType)
            else: 
                print("BARF")
...
#           element(elasticBeamColumn eleTag end1 end2 A E I 1 -mass M
    ...
    if testOK == 0:
        print("PASSED Verification Test EigenFrame.Extra.py", eleType, "  \n\n")
    else :
        print("FAILED Verification Test EigenFrame.Extra.py", eleType, "  \n\n")
...
#           element(elasticBeamColumn eleTag end1 end2 A E I 1 -mass M
    ...
    # calculate eigenvalues
    numEigen = 3
    eigenValues =eigen(solverType, numEigen)
    PI = 2*asin(1.0)
