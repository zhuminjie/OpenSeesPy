...
for eleType in ['stdBrick', 'SSPbrick']:
    ...
    for numFloor in [6,3,1]:
        for numBay in [1,3,6]:
            ...
            load(nodeTop+(nx+1)*(ny+1), 50.0, 0.0, 0.0)
            ...
            for i in range(1,numFloor+1):
                ...
                for j in range(1,nx+1):
                    equalDOF(mNode1, mNode1+j, 1)
                    equalDOF(mNode2, mNode2+j, 1)
...
if testOK == 0:
    print("PASSED Verification Test PlanarShearWall.py \n\n")
else:
    print("FAILED Verification Test PlanarShearWall.py \n\n")
